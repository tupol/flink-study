 ...
 *
 * A! Important! In this one no task is taking longer than the timeout.
 */
class WithoutRealTimeout extends FunSuite with Matchers with BeforeAndAfterAll {

  val TIMEOUT = 500 milli
  ...
  val inputRecords = generateRandomRecords(size = 1000, maxDuration = TIMEOUT.toMillis - 100)
  ...
  test("Output Differences") {
    ...
    tmapRuntime should equal(adsoRuntime +- 1000)
    tmapRuntime should equal(adsnRuntime +- 1000)
    ...
    linesTMAP.foldLeft(false)((acc, line) => acc || line.contains("Failure")) shouldBe false

    linesADSO shouldBe linesTMAP
